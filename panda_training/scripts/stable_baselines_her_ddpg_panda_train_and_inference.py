...
`her wrapper <https://stable-baselines.readthedocs.io/en/master/modules/her.html>`_.
"""
...
try:
    from stable_baselines import DDPG, HER  # HER, TD3, SAC
except ImportError:
    if __name__ == "__main__":
        rospy.logwarn(
            "The stable-baslines HER example can not be run the mpi requirements "
...
MODEL_LOAD_FILE_NAME = (
    "her-ddpg-pandapush-v0-1.zip"  # The file name of the model you want to load
)
...
MODEL_CLASS = DDPG  # The learning model. Works also with sac, ddpg and TD3
...
if LOAD:  # If the user want to load an existing model
    ...
    MODEL_DIR = os.path.abspath(
        os.path.join(
            FILE_PATH,
            "../models/{}/stable_baselines/her".format(TASK_ENV_NAME.lower()),
        )
    ...
    MODEL_NAME = "her-ddpg-{}-".format(TASK_ENV_NAME.lower()) + get_unique_file_suffix(
...
TB_LOGDIR = os.path.abspath(
    os.path.join(
        FILE_PATH, "../logs/{}/stable-baselines/her".format(TASK_ENV_NAME.lower())
    )
)
...
if __name__ == "__main__":
    ...
    # Initialize ros node
    rospy.init_node(
        "stable_baselines_her_panda_train_and_inference", log_level=rospy.INFO
    )
    ...
    if LOAD:
        ...
        rospy.sleep(2)
        model = HER.load(MODEL_FILE, env=env, tensorboard_log=TB_LOGDIR)
    else:

        # Create the HER model
        model = HER(
            MODEL_POLICY,
            env,
            MODEL_CLASS,
