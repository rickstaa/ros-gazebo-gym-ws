...
from stable_baselines import SAC  # DDPG, TD3, HER
...
MODEL_LOAD_FILE_NAME = (
    "sac-pandapush-v0-1.zip"  # The file name of the model you want to load
)
...
if LOAD:  # If the user want to load an existing model
    ...
    MODEL_DIR = os.path.abspath(
        os.path.join(
            FILE_PATH,
            "../models/{}/stable_baselines/sac".format(TASK_ENV_NAME.lower()),
        )
    ...
    MODEL_NAME = "sac-{}-".format(TASK_ENV_NAME.lower()) + get_unique_file_suffix(
...
TB_LOGDIR = os.path.abspath(
    os.path.join(
        FILE_PATH, "../logs/{}/stable-baselines/sac".format(TASK_ENV_NAME.lower())
    )
)
...
if __name__ == "__main__":
    ...
    # NOTE: As the SAC algorithm only accepts a Gazebo Env and not a Gazebo Goal env
    ...
    if LOAD:
        ...
        rospy.sleep(2)
        model = SAC.load(MODEL_FILE, env=env, tensorboard_log=TB_LOGDIR)
    else:

        # Create the sac model
        model = SAC(MODEL_POLICY, env, verbose=1, tensorboard_log=TB_LOGDIR)
